...
1. Generate the token for the desired tenant that will be used for Authorization.
   ...
   ```console
    dellctl generate token --addr csm-authorization.com:<port> --insecure true --tenant <tenant> --access-token-expiration 30m0s --refresh-token-expiration 1480h0m0s | k -n vxflexos apply -f -
   ```

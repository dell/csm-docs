...
For a cluster with multiple network interfaces per cluster node, you can specify the `AZNetwork` in the storage class to configure the NFS export with all of the associated IP addresses. See the storage class snippets below for examples of storage classes using this feature.

```yaml
# This is a snippet of a storage class for utilizing the AZNetwork feature.

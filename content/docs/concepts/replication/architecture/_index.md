...
`multi-cluster` The PVC objects are replicated across the clusters.

`single cluster` claimRef params is updated on remove PV with `name` and `namespace` as `reserved` by default to prevent the remote PV being available to everyone.

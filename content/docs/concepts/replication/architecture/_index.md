...
`multi-cluster` The PVC objects are replicated across the clusters. (provided `allowPvcCreationOnTarget` or `REPLICATION_ALLOW_PVC_CREATION_ON_TARGET` is set to `true`)

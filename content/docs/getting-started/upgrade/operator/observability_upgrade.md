...
- Modifying the existing driver and module installation directly via `kubectl edit`
  ...
  For example - If the CSI driver is installed then run this command to get the object name

  ```bash
  # Replace driver-namespace with the namespace where the CSI driver is installed
...
- Upgrade csm-operator and csi-driver first which support the corresponding observability module version.
{{< /hide >}}

{{< hide id="2" >}}

  For example - If the object name is powermax then use the name as powermax

  ```bash
  # Replace object-name with the powermax
  kubectl edit csm powermax -n <driver-namespace>
  ```

- Modify the installation

  - Update the driver config version and image tag
  - Update the Observability config version and the driver metrics images(e.g. for CSI PowerMax driver, the metrics driver image would be `csm-metrics-powermax`)
  - Follow the additional steps to upgrade to Observability module v1.13.0:
      - Remove the karavi-topology component:
      ...
      - Configure Environment Variables:
          ...
          To enable and customize the collection of topology data, add the following environment variables to your custom resource. Or apply the [latest custom resource](https://github.com/dell/csm-operator/blob/main/samples/v2.16.0/). By default, `POWERMAX_TOPOLOGY_METRICS_ENABLED` is true and `POWERMAX_TOPOLOGY_METRICS_POLL_FREQUENCY` is 30 seconds.
          ```bash
      ...
      - Delete older observability version deployments and services from karavi namespace.
        ```bash
        kubectl delete deployment karavi-metrics-powermax -n karavi
        kubectl delete deployment karavi-topology -n karavi
        kubectl delete deployment otel-collector -n karavi
        kubectl delete service karavi-metrics-powermax -n karavi
        kubectl delete service karavi-topology -n karavi
        kubectl delete service otel-collector -n karavi
        ```
...
- Modify the installation
  ...
  - Update the driver config version and image tag
  - Update the Observability config version and the driver metrics images(e.g. for CSI PowerStore driver, the metrics driver image would be `csm-metrics-powerstore`)
  - Follow the additional steps to upgrade to Observability module v1.13.0:
      - Remove the karavi-topology component:
      ...
      - Configure Environment Variables:
          ...
          To enable and customize the collection of topology data, add the following environment variables to your custom resource. Or apply the [latest custom resource](https://github.com/dell/csm-operator/blob/main/samples/v2.16.0/). By default, `POWERSTORE_TOPOLOGY_METRICS_ENABLED` is true and `POWERSTORE_TOPOLOGY_POLL_FREQUENCY` is 30 seconds.
          ```bash
      ...
      - Delete older observability version deployments and services from karavi namespace.
        ```bash
        kubectl delete deployment karavi-metrics-powerstore -n karavi
        kubectl delete deployment karavi-topology -n karavi
        kubectl delete deployment otel-collector -n karavi
        kubectl delete service karavi-metrics-powerstore -n karavi
        kubectl delete service karavi-topology -n karavi
        kubectl delete service otel-collector -n karavi
        ```
...
- Modify the installation
  ...
  - Update the driver config version and image tag
  - Update the Observability config version and the driver metrics images(e.g. for CSI PowerFlex driver, the metrics driver image would be `karavi-metrics-powerflex`)
  - Follow the additional steps to upgrade to Observability module v1.13.0:
      - Remove the karavi-topology component:
      ...
      - Configure Environment Variables:
          ...
          To enable and customize the collection of topology data, add the following environment variables to your custom resource. Or apply the [latest custom resource](https://github.com/dell/csm-operator/blob/main/samples/v2.16.0/). By default, `POWERFLEX_TOPOLOGY_METRICS_ENABLED` is true and `POWERFLEX_TOPOLOGY_METRICS_POLL_FREQUENCY` is 30 seconds.
          ```bash

...
| X_CSI_NFS_CLIENT_PORT      | Define port for the HBNFS client. | No | "2050" |
| X_CSI_NFS_SERVER_PORT      | Define port for the HBNFS server. This value should match what port the nfs-server is configured on. See /etc/nfs.conf on the worker nodes for port information. | No | "2049" |
| X_CSI_NFS_EXPORT_DIRECTORY | Define mount path for HBNFS volumes. | No | "/var/lib/dell/nfs" |

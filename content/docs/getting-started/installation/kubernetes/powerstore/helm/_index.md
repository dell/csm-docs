...
| nfsExportDirectory | Define mount path for HBNFS volumes. | No | "/var/lib/dell/nfs" |
| nfsServerPort | Define port for the HBNFS server. This value should match what port the nfs-server is configured on. See /etc/nfs.conf on the worker nodes for port information. | No | "2049" |
| nfsClientPort | Define port for the HBNFS client. | No | "2050" |

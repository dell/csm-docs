...
| nfsExportDirectory | Define mount path for Host Based NFS volumes. | No | "/var/lib/dell/nfs" |
| nfsServerPort | Define port for the Host Based NFS server. This value should match what port the nfs-server is configured on. See /etc/nfs.conf on the worker nodes for port information. | No | "2049" |
| nfsClientPort | Define port for the Host Based NFS client. | No | "2050" |

 ...
 * As a pre-requisite for running this procedure, you must have the downloaded files, including the Helm chart from the source [git repository](https://github.com/dell/csi-unity) with the command
   ```bash
   git clone -b {{< version-docs key="csi_unity_latest_version" >}} https://github.com/dell/csi-unity.git
   ```
...
3. Edit `values.yaml` to set the following parameters for your installation:
    ...
    The following table lists the primary configurable parameters of the Unity XT driver chart and their default values. More detailed information can be found in the [`values.yaml`](https://github.com/dell/helm-charts/blob/csi-unity-2.15.0/charts/csi-unity/values.yaml) file in this repository.
<ul>
...
8. You can also install the driver using standalone helm chart by cloning the centralised helm charts and running the helm install command as shown.
   ...
   **Syntax**:
   ```bash
   git clone -b csi-unity-2.15.0 https://github.com/dell/helm-charts

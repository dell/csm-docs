...
>NOTE: The CSM Installation Wizard generates `values.yaml` with the minimal inputs required to install the CSM. To configure additional parameters in values.yaml, you can follow the steps outlined in [PowerStore](../../powerstore/helm#install-the-driver), [PowerMax](../../powermax/helm#install-the-driver), [PowerScale](../../powerscale/helm#install-the-driver), [PowerFlex](../../powerflex/helm#install-the-driver), [Unity XT](../../unityxt/helm#install-csi-driver), [Observability](docs/getting-started/installation/helm/modules/observability), [Replication](docs/getting-started/installation/helm/modules/replication/), [Resiliency](docs/getting-started/installation/helm/modules/resiliency/).
...
5. If Observability is checked in the wizard, refer to [Observability](docs/getting-started/installation/helm/modules/observability) to export metrics to Prometheus and load the Grafana dashboards.

6. If Authorization is checked in the wizard, only the sidecar is enabled. Refer to [Authorization](docs/getting-started/installation/helm/modules/authorizationv2.0/) to install and configure the CSM Authorization Proxy Server.

7. If Replication is checked in the wizard, refer to [Replication](docs/getting-started/installation/helm/modules/replication/) on configuring communication between Kubernetes clusters.

8. If your Kubernetes distribution doesn't have the Volume Snapshot feature enabled, refer to [this section](docs/concepts/snapshots) to install the Volume Snapshot CRDs and the default snapshot controller.

...
4. Copy only the deployed CSI driver entities to the Observability namespace
    ...
    2. Copy the secrets in `powermax-reverseproxy-config` from the CSI Driver for PowerMax namespace to the CSM namespace.
       ...
       If [Container Storage Modules for Authorization is enabled](docs/getting-started/installation/helm/modules/authorizationv2#configuring-a-dell-csi-driver-with-container-storage-module-for-authorization) for CSI PowerMax, perform these steps:

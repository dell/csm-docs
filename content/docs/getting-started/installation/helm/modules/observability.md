...
4. Copy only the deployed CSI driver entities to the Observability namespace
    ...
    1. Copy the config Secret from the CSI PowerScale namespace into the Container Storage Modules for Observability namespace:
    ...
    4. Copy the `proxy-server-root-certificate` and `proxy-authz-tokens` secrets from the CSI PowerMax namespace into the Container Storage Modules for Observability namespace:
...
5. Configure the [parameters](#configuration) and install the Container Storage Modules for Observability Helm Chart

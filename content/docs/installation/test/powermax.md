...
3. Run the starttest.sh script and provide it with a test name. The following is a sample command that can be used to run the _2vols_ test: `./starttest.sh -t 2vols -n <test_namespace> -s <storageclass-name>`
...
>*NOTE*: Helm tests have been designed assuming that users are created storageclass names like <storageclass-name> and <storageclass-name-xfs>. You can use `kubectl get sc` to check for the storageclass names.

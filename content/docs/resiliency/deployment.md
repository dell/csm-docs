...
| ignoreVolumelessPods | Optional | Boolean value that if set to true will enable CSM for Resiliency to ignore pods without persistent volume attach to it even if those pods has CSM for Resiliency label on it. | controller & node |

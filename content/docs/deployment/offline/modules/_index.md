...
3. Copy the CSI Driver Secret(s)
    ...
    If [CSM for Authorization is enabled](../../../deployment/helm/modules/installation/authorization-v2.x/#configuring-a-dell-csi-driver-with-csm-for-authorization) for CSI PowerFlex, perform these steps:
   ...
   kubectl get configmap [VXFLEXOS-CONFIG-PARAMS] -n [CSI_DRIVER_NAMESPACE] -o yaml | sed 's/name: [VXFLEXOS-CONFIG-PARAMS]/name: vxflexos-config-params/' | sed 's/namespace: [CSI_DRIVER_NAMESPACE]/namespace: [CSM_NAMESPACE]/' | kubectl create -f -
    ...
    If [CSM for Authorization is enabled](../../../deployment/helm/modules/installation/authorization-v2.x/#configuring-a-dell-csi-driver-with-csm-for-authorization) for CSI PowerScale, perform these steps:
    ...
    If [CSM for Authorization is enabled](../../../deployment/helm/modules/installation/authorization-v2.x/#configuring-a-dell-csi-driver-with-csm-for-authorization) for CSI PowerMax, perform these steps:

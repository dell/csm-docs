...
2. Create *isilon-creds* secret by creating a yaml file called secret.yaml with the following content:
   ...
   Replace the values for the given keys as per your environment. If replication feature is enabled, ensure the secret includes all the PowerScale clusters involved in replication. After creating the secret.yaml, the following command can be used to create the secret,

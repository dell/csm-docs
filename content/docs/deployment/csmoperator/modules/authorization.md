...
3. Users should configure the parameters in the CR. The following table lists the primary configurable parameters of the Authorization Proxy Server and their default values:
...
4. Execute the following command to create Authorization CR:
...
5. Create the `karavi-auth-tls` secret using your own certificate or by using a self-signed certificate generated via cert-manager. 
    ...
    If using your own certificate that is valid for each Ingress hostname, use the following command to create the `karavi-auth-tls` secret:
    ...
    Use the following command to create the `karavi-auth-tls` secret:

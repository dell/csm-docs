...
3. Prepare `samples/authorization/config.yaml` provided [here](https://github.com/dell/csm-operator/tree/release/v1.8.0/authorization/config.yaml) which contains the JWT signing secret. The following table lists the configuration parameters.
...
4. Create the `karavi-storage-secret` using the file provided [here](https://github.com/dell/csm-operator/tree/release/v1.8.0/authorization/karavi-storage-secret.yaml) to store storage system credentials.
...
2. Create a CR (Custom Resource) for Authorization using the sample file provided [here](https://github.com/dell/csm-operator/tree/release/v1.8.0/authorization/csm_authorization_proxy_server_v170.yaml). This file can be modified to use custom parameters if needed.
...
5. Create the `karavi-auth-tls` secret using your own certificate or by using a self-signed certificate generated via cert-manager. 
    ...
    If using a self-signed certificate, prepare `samples/authorization/certificate_v170.yaml` provided [here](https://github.com/dell/csm-operator/tree/release/v1.8.0/authorization/certificate_v170.yaml). An entry for each hostname specified in the CR must be added under `dnsNames` for the certificate to be valid for each Ingress. 

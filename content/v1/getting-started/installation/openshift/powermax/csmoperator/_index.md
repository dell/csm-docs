...
5. **Create a CR (Custom Resource)** for PowerMax using the sample files provided
    ...
    a. **Minimal Configuration:** Use the [sample file](https://github.com/dell/csm-operator/tree/release/v1.9.1/samples/minimal-samples/powermax_{{< version-v1 key="Min_sample_operator_pmax" >}}.yaml) for default settings. If using the secret above, ensure that the secret name of the secret created is `powermax-creds`.

    [OR]

    b. **Detailed Configuration:** Use the [sample file](https://github.com/dell/csm-operator/tree/release/v1.9.1/samples/storage_csm_powermax_{{< version-v1 key="Det_sample_operator_pmax" >}}.yaml) for detailed settings or use [Wizard](./installationwizard#generate-manifest-file) to generate the sample file. 
    .

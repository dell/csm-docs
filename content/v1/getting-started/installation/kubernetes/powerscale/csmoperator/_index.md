...
4. **Install Driver**
   ...
   i. **Create a CR (Custom Resource)** for PowerScale using the sample files provided
    ...
    b. **Detailed Configuration:** Use the [sample file](https://github.com/dell/csm-operator/blob/release/{{< version-v1 key="csm-operator_latest_version">}}/samples/{{< version-v1 key="csm-operator_latest_samples_dir" >}}/storage_csm_powerscale_{{< version-v1 key="Det_sample_operator_pscale" >}}.yaml) for detailed settings or use [Wizard](./installationwizard#generate-manifest-file) to generate the sample file.

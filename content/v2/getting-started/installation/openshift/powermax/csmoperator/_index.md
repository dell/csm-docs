...
<br>
{{<include  file="content/v2/getting-started/installation/openshift/powermax/prerequisite/_index.md" >}}
...
5. **Create a CR (Custom Resource)** for PowerMax using the sample files provided
    ...
    a. **Minimal Configuration:** Use the [sample file](https://github.com/dell/csm-operator/blob/main/samples/minimal-samples/powermax_{{< version-v2 key="Min_sample_operator_pmax" >}}.yaml) for default settings. If using the secret above, ensure that the secret name of the secret created is `powermax-creds`.

    [OR]

    b. **Detailed Configuration:** Use the [sample file](https://github.com/dell/csm-operator/blob/main/samples/storage_csm_powermax_{{< version-v2 key="Det_sample_operator_pmax" >}}.yaml) for detailed settings or use [Wizard](./installationwizard#generate-manifest-file) to generate the sample file. 
    .
...
{{< collapse id="1" title="Parameters">}}
  ...
  powermax    3h             powermax        {{< version-v2 key="PMax_latestVersion" >}}         Succeeded      
  ```
